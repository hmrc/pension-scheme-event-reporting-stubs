...
class HipEventReportControllerSpec extends SpecBase {

  import HipEventReportController.*
  ...
  private val controller = app.injector.instanceOf[HipEventReportController]
  ...
  "api1832GET" must {
    ...
    Seq("Event2", "Event3", "Event4", "Event5", "Event6", "Event7", "Event8", "Event8A", "Event22", "Event23", "Event24").foreach {
      event =>
        s"return 200 for a valid request, $event" in {
  ...
  "api1831GET" must {
    ...
    "return InternalServerError" in {
      ...
      running() { app =>
        val controller = app.injector.instanceOf[HipEventReportController]
        val result = controller.api1831GET(pstr = "21000001AA")(getRequest)
    ...
    "return ServiceUnavailable" in {
      ...
      running() { app =>
        val controller = app.injector.instanceOf[HipEventReportController]
        val result = controller.api1831GET(pstr = "21000002AA")(getRequest)
    ...
    "return DUPLICATE_SUBMISSION" in {
      ...
      running() { app =>
        val controller = app.injector.instanceOf[HipEventReportController]
        val result = controller.api1831GET(pstr = "24000009IN")(getRequest)
    ...
    "return INVALID_PAYLOAD" in {
      ...
      running() { app =>
        val controller = app.injector.instanceOf[HipEventReportController]
        val result = controller.api1831GET(pstr = "21000004AA")(getRequest)
    ...
    "return REQUEST_NOT_PROCESSED" in {
      ...
      running() { app =>
        val controller = app.injector.instanceOf[HipEventReportController]
        val result = controller.api1831GET(pstr = "21000005AA")(getRequest)
  ...
  "api1833GET" must {
    "return 200 OK for a valid request" in {
      val validData = readJsonFromFile(filePath = "/resources/data/api1833/87219363YN.json")
    ...
    "return 404 NOT FOUND for a not found PSTR" in {
      val fakeRequest =
        FakeRequest(method = "GET", path = "/").withHeaders(
          ("CorrelationId", "testId"),
          ("Authorization", "test Bearer token"),
          ("Environment", "local"),
          ("reportVersionNumber", "version"),
      ...
      running() { _ =>
        val result: Future[Result] = controller.api1833GET(pstr = "24000007IN")(fakeRequest)
